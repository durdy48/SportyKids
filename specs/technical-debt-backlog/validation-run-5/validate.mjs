#!/usr/bin/env node
/**
 * Validation Script — Run 5
 * PRD1 regression + Appendix A + PRD2 + Appendix B + PRD3 (Parental Trust, Daily Missions, Dark Mode)
 */
...
const ASSETS = path.resolve(__dirname, '../validation-assets/run-5');
...
async function validateParentalSessions() {
  ...
  try {
    const userRes = await fetchJson('http://localhost:3099/api/users', {
      ...
      body: JSON.stringify({ name: 'Validation Kid R5', age: 10, favoriteSports: ['football'] }),
    });
...
async function validatePostgreSQL() {
  ...
  try {
    const res = await fetchJson('http://localhost:3099/api/users', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: 'PG Test User R5', age: 11, favoriteSports: ['football', 'basketball'] }),
    });
...
function validateTestCounts() {
  // T.42: API >= 380, Web >= 60, Mobile >= 60
  const total = testCounts.api + testCounts.web + testCounts.mobile;
  const pass = testCounts.api >= 380 && testCounts.web >= 60 && testCounts.mobile >= 60 && total >= 500;
  record('T.42', 'Test count thresholds', pass ? 'PASS' : 'FAIL',
    `API: ${testCounts.api} (>=380), Web: ${testCounts.web} (>=60), Mobile: ${testCounts.mobile} (>=60), Total: ${total} (>=500)`);
...
async function main() {
  // eslint-disable-next-line no-console
  console.log('=== Validation Run 5 — PRD1 + Appendix A + PRD2 + Appendix B + PRD3 ===\n');
  ...
  let report = `# Validation Report — Run 5 (PRD3: Parental Trust, Daily Missions, Dark Mode)\n\n`;
  ...
  report += `- Run 5: ${passCount} PASS / ${failCount} FAIL / ${skipCount} SKIP (PRD1 + Appendix A + PRD2 + Appendix B + PRD3)\n`;
  ...
  // Evidence links
  report += `## Evidence\n\n`;
  report += `- [Test results](run-5/output/10-test-all.txt)\n`;
  report += `- [ESLint output](run-5/output/04-eslint-output.txt)\n`;
  report += `- [API startup logs](run-5/output/01-api-startup-logs.txt)\n`;
  report += `- [Parental session tests](run-5/output/14-parental-session-tests.txt)\n`;
  report += `- [Docker status](run-5/output/20-docker-ps.txt)\n`;
  report += `- [JSON.parse audit](run-5/output/24-json-parse-audit.txt)\n`;
  report += `- [Dead code grep](run-5/output/33-dead-code-grep.txt)\n`;
  report += `- [Web typecheck](run-5/output/36-web-typecheck.txt)\n`;
  report += `- [Error handler details](run-5/output/44-error-handler-details.txt)\n`;
  report += `- [Typed errors audit](run-5/output/47-typed-errors-audit.txt)\n`;
  report += `- [NOT_FOUND response](run-5/api/27-not-found.json)\n`;
  report += `- [Validation error response](run-5/api/28-validation-error.json)\n`;
  report += `- [User creation (PG arrays)](run-5/api/23-create-user-pg.json)\n`;
  report += `- [Trending endpoint](run-5/api/26-trending.json)\n`;
  report += `- [Digest GET](run-5/api/51-digest-get.json)\n`;
  report += `- [Digest PUT](run-5/api/52-digest-put.json)\n`;
  report += `- [Digest test endpoint](run-5/api/53-digest-test.json)\n`;
  report += `- [Missions today](run-5/api/66-missions-today.json)\n`;
  report += `- [LimitReached web](run-5/output/61-limit-reached-web.txt)\n`;
  report += `- [Dark mode components](run-5/output/77-dark-mode-components.txt)\n`;
  report += `- [en.json i18n keys](run-5/output/87-en-i18n-keys.txt)\n`;
  report += `- [es.json i18n keys](run-5/output/88-es-i18n-keys.txt)\n`;

  fs.writeFileSync(path.join(ASSETS, '..', 'validation-report-run-5.md'), report, 'utf-8');
