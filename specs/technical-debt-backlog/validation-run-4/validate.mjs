 ...
 * PRD1 regression + Appendix A + PRD2 + Appendix B (post /t-review #2 fixes)
 */
...
const ASSETS = path.resolve(__dirname, '../validation-assets/run-4');
...
function validateAppendixB() {
  ...
  try {
    ...
    const moduleRequire = vp.includes("require('expo-video')") && !vp.includes('function ExpoVideoPlayer');
    // Check that require is at module scope (before component definition)
    const requireIdx = vp.indexOf("require('expo-video')");
    const componentIdx = vp.indexOf('function ExpoVideoPlayer') !== -1 ? vp.indexOf('function ExpoVideoPlayer') : vp.indexOf('ExpoVideoPlayer');
    const isModuleScope = requireIdx < componentIdx || requireIdx < 100;
...
async function main() {
  // eslint-disable-next-line no-console
  console.log('=== Validation Run 4 — PRD1 + Appendix A + PRD2 + Appendix B ===\n');
  ...
  let report = `# Validation Report — Run 4 (post /t-review #2)\n\n`;
  ...
  report += `- Run 4: ${passCount} PASS / ${failCount} FAIL / ${skipCount} SKIP (PRD1 + Appendix A + PRD2 + Appendix B)\n`;
  ...
  // Evidence links
  report += `## Evidence\n\n`;
  report += `- [Test results](run-4/output/10-test-all.txt)\n`;
  report += `- [ESLint output](run-4/output/04-eslint-output.txt)\n`;
  report += `- [API startup logs](run-4/output/01-api-startup-logs.txt)\n`;
  report += `- [Parental session tests](run-4/output/14-parental-session-tests.txt)\n`;
  report += `- [Docker status](run-4/output/20-docker-ps.txt)\n`;
  report += `- [JSON.parse audit](run-4/output/24-json-parse-audit.txt)\n`;
  report += `- [Dead code grep](run-4/output/33-dead-code-grep.txt)\n`;
  report += `- [Web typecheck](run-4/output/36-web-typecheck.txt)\n`;
  report += `- [Error handler details](run-4/output/44-error-handler-details.txt)\n`;
  report += `- [Typed errors audit](run-4/output/47-typed-errors-audit.txt)\n`;
  report += `- [NOT_FOUND response](run-4/api/27-not-found.json)\n`;
  report += `- [Validation error response](run-4/api/28-validation-error.json)\n`;
  report += `- [User creation (PG arrays)](run-4/api/23-create-user-pg.json)\n`;
  report += `- [Trending endpoint](run-4/api/26-trending.json)\n`;

  fs.writeFileSync(path.join(ASSETS, '..', 'validation-report-run-4.md'), report, 'utf-8');
