/**
 * Admin Dashboard — Validation Script Run 6 (post /t-review #3)
 ...
 *
 * Generates report: specs/admin-dashboard/validation-assets/validation-report-run-6.md
 */
...
const RUN_DIR = resolve(ASSETS_DIR, 'run-6');
const API_DIR = resolve(RUN_DIR, 'api');
const OUTPUT_DIR = resolve(RUN_DIR, 'output');
const REPORT_PATH = resolve(ASSETS_DIR, 'validation-report-run-6.md');
...
// ===========================================================================
// SECTION 1: Re-run of original checks (mirrors Run 5 exactly, paths → run-6)
// ===========================================================================
...
async function runA02BatchMixedTypes(adminToken) {
  ...
  if (pendingNews.length === 0 || pendingReels.length === 0) {
    ...
    if (newsOk && reelOk) {
      record('Appendix A', 'C2: Batch validates both news and reel types (400 on missing reason)', 'PASS', 'type=news → 400, type=reel → 400', 'run-6/api/A02-batch-types.json');
    } else {
      record('Appendix A', 'C2: Batch validates both news and reel types (400 on missing reason)', 'FAIL', `news400=${newsOk}, reel400=${reelOk}`, 'run-6/api/A02-batch-types.json');
    }
    return;
  }
  ...
  savePayload('A02-batch-types.json', result);
  if (newsOk && reelOk) {
    record('Appendix A', 'C2: Batch accepts news and reel types independently', 'PASS', 'news: ok, reel: ok', 'run-6/api/A02-batch-types.json');
  } else {
    record('Appendix A', 'C2: Batch accepts news and reel types independently', 'FAIL', `news=${newsOk}, reel=${reelOk}`, 'run-6/api/A02-batch-types.json');
  }
}
...
function runA04ApiBaseDuplicate() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix A', 'W1: No duplicate API_BASE in moderation page', 'FAIL', 'File not found', 'run-6/output/A04-api-base-check.txt');
    return;
  }
  try {
    ...
    saveOutput('A04-api-base-check.txt', output);
    if (matches.length === 0) {
      record('Appendix A', 'W1: No duplicate API_BASE in moderation page', 'PASS', 'const API_BASE not found (uses centralized import)', 'run-6/output/A04-api-base-check.txt');
    } else if (matches.length === 1) {
      record('Appendix A', 'W1: No duplicate API_BASE in moderation page', 'PASS', 'Only 1 const API_BASE declaration', 'run-6/output/A04-api-base-check.txt');
    } else {
      record('Appendix A', 'W1: No duplicate API_BASE in moderation page', 'FAIL', `Found ${matches.length} occurrences`, 'run-6/output/A04-api-base-check.txt');
    }
  } catch (err) {
    saveOutput('A04-api-base-check.txt', `Error: ${err.message}`);
    record('Appendix A', 'W1: No duplicate API_BASE in moderation page', 'FAIL', err.message, 'run-6/output/A04-api-base-check.txt');
  }
}
...
function runA05TableKeyStability() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix A', 'W2: AdminTable does not use index as key', 'FAIL', 'File not found', 'run-6/output/A05-table-key-check.txt');
    return;
  }
  try {
    const content = readFileSync(filePath, 'utf8');
    ...
    if (dataRowBadKeys.length === 0 && hasStableDataKey) {
      record('Appendix A', 'W2: AdminTable does not use index as key', 'PASS', 'Data rows use stable id-based key', 'run-6/output/A05-table-key-check.txt');
    } else {
      record('Appendix A', 'W2: AdminTable does not use index as key', 'FAIL', `dataRowBadKeys=${dataRowBadKeys.length}, stableKey=${hasStableDataKey}`, 'run-6/output/A05-table-key-check.txt');
    }
  } catch (err) {
    saveOutput('A05-table-key-check.txt', `Error: ${err.message}`);
    record('Appendix A', 'W2: AdminTable does not use index as key', 'FAIL', err.message, 'run-6/output/A05-table-key-check.txt');
  }
}
...
function runA07AriaCurrentSidebar() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix A', 'W5: AdminSidebar uses aria-current', 'FAIL', 'File not found', 'run-6/output/A07-aria-current-check.txt');
    return;
  }
  try {
    const content = readFileSync(filePath, 'utf8');
    ...
    saveOutput('A07-aria-current-check.txt', output);
    if (matches.length > 0) {
      record('Appendix A', 'W5: AdminSidebar uses aria-current', 'PASS', `Found ${matches.length} aria-current attribute(s)`, 'run-6/output/A07-aria-current-check.txt');
    } else {
      record('Appendix A', 'W5: AdminSidebar uses aria-current', 'FAIL', 'No aria-current found', 'run-6/output/A07-aria-current-check.txt');
    }
  } catch (err) {
    saveOutput('A07-aria-current-check.txt', `Error: ${err.message}`);
    record('Appendix A', 'W5: AdminSidebar uses aria-current', 'FAIL', err.message, 'run-6/output/A07-aria-current-check.txt');
  }
}
...
function runA08CryptoPassword() {
  ...
  if (!existsSync(scriptPath)) {
    ...
    record('Appendix A', 'S2: Password uses crypto.randomBytes (not Math.random)', 'FAIL', 'Script not found', 'run-6/output/A08-crypto-password-check.txt');
    return;
  }
  try {
    ...
    if (hasCryptoRandomBytes && !hasMathRandom) {
      record('Appendix A', 'S2: Password uses crypto.randomBytes (not Math.random)', 'PASS', 'crypto.randomBytes: ✓, Math.random: ✗', 'run-6/output/A08-crypto-password-check.txt');
    } else if (!hasCryptoRandomBytes) {
      record('Appendix A', 'S2: Password uses crypto.randomBytes (not Math.random)', 'FAIL', 'crypto.randomBytes not found', 'run-6/output/A08-crypto-password-check.txt');
    } else {
      record('Appendix A', 'S2: Password uses crypto.randomBytes (not Math.random)', 'FAIL', 'Math.random still present', 'run-6/output/A08-crypto-password-check.txt');
    }
  } catch (err) {
    saveOutput('A08-crypto-password-check.txt', `Error: ${err.message}`);
    record('Appendix A', 'S2: Password uses crypto.randomBytes (not Math.random)', 'FAIL', err.message, 'run-6/output/A08-crypto-password-check.txt');
  }
}
...
function runA09ErrorState() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix A', 'S5: Moderation page has role="alert" for errors', 'FAIL', 'File not found', 'run-6/output/A09-error-state-check.txt');
    return;
  }
  try {
    const content = readFileSync(filePath, 'utf8');
    ...
    saveOutput('A09-error-state-check.txt', output);
    if (hasRoleAlert) {
      record('Appendix A', 'S5: Moderation page has role="alert" for errors', 'PASS', 'role="alert" found in moderation page', 'run-6/output/A09-error-state-check.txt');
    } else {
      record('Appendix A', 'S5: Moderation page has role="alert" for errors', 'FAIL', 'role="alert" not found', 'run-6/output/A09-error-state-check.txt');
    }
  } catch (err) {
    saveOutput('A09-error-state-check.txt', `Error: ${err.message}`);
    record('Appendix A', 'S5: Moderation page has role="alert" for errors', 'FAIL', err.message, 'run-6/output/A09-error-state-check.txt');
  }
}
...
function runC05AllSystemsOperational() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Part C', 'C5: Overview shows "All systems operational" when alerts=[]', 'FAIL', 'File not found', 'run-6/output/C05-all-systems-check.txt');
    return;
  }
  try {
    const content = readFileSync(filePath, 'utf8');
    ...
    if (hasAlertsLengthCheck && hasAllSystemsText) {
      record('Part C', 'C5: Overview shows "All systems operational" when alerts=[]', 'PASS', 'Condition and text both present in source', 'run-6/output/C05-all-systems-check.txt');
    } else {
      record('Part C', 'C5: Overview shows "All systems operational" when alerts=[]', 'FAIL', `alerts.length check=${hasAlertsLengthCheck}, text=${hasAllSystemsText}`, 'run-6/output/C05-all-systems-check.txt');
    }
  } catch (err) {
    saveOutput('C05-all-systems-check.txt', `Error: ${err.message}`);
    record('Part C', 'C5: Overview shows "All systems operational" when alerts=[]', 'FAIL', err.message, 'run-6/output/C05-all-systems-check.txt');
  }
}
...
function runC06PendingContentAlert() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Part C', 'C6: pending_content alert fires for items >30min old', 'FAIL', 'admin.ts not found', 'run-6/output/C06-pending-alert-check.txt');
    return;
  }
  const content = readFileSync(filePath, 'utf8');
  ...
  if (hasBuildAlerts && has30minCheck && hasPendingType && hasWarningSeverity) {
    record('Part C', 'C6: pending_content alert fires for items >30min old', 'PASS', 'buildAlerts() has 30-min threshold, pending_content type, and warning severity verified in source', 'run-6/output/C06-pending-alert-check.txt');
  } else {
    const missing = [];
    ...
    record('Part C', 'C6: pending_content alert fires for items >30min old', 'FAIL', missing.join(', '), 'run-6/output/C06-pending-alert-check.txt');
  }
}
...
function runC07PendingCriticalSourceCheck() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Part C', 'C7: admin.ts has pendingTotal > 50 → pending_content_critical', 'FAIL', 'File not found', 'run-6/output/C07-pending-critical-check.txt');
    return;
  }
  try {
    const content = readFileSync(filePath, 'utf8');
    ...
    if (hasCriticalCondition && hasCriticalType && hasErrorSeverity) {
      record('Part C', 'C7: admin.ts has pendingTotal > 50 → pending_content_critical', 'PASS', 'Condition, type, and severity all present in source', 'run-6/output/C07-pending-critical-check.txt');
    } else {
      record('Part C', 'C7: admin.ts has pendingTotal > 50 → pending_content_critical', 'FAIL', `condition=${hasCriticalCondition}, type=${hasCriticalType}, severity=${hasErrorSeverity}`, 'run-6/output/C07-pending-critical-check.txt');
    }
  } catch (err) {
    saveOutput('C07-pending-critical-check.txt', `Error: ${err.message}`);
    record('Part C', 'C7: admin.ts has pendingTotal > 50 → pending_content_critical', 'FAIL', err.message, 'run-6/output/C07-pending-critical-check.txt');
  }
}
...
async function runBW1PendingAlertImmediate(adminToken) {
  ...
  if (!existsSync(adminTsPath)) {
    ...
    record('Appendix B', 'W1: pending_content alert fires immediately (no 30-min gate)', 'FAIL', 'admin.ts not found', 'run-6/output/B01-pending-alert-immediate.txt');
    return;
  }
  ...
  try {
    ...
    if (hasBuildAlerts && hasPendingTotalCheck && hasPendingContentType && hasNewestJustArrived && hasForOver30Min) {
      record('Appendix B', 'W1: pending_content alert fires immediately (no 30-min gate)', 'PASS',
        ...
        'run-6/output/B01-pending-alert-immediate.txt');
    } else {
      const missing = [];
      if (!hasBuildAlerts) missing.push('buildAlerts() not found');
    ...
    record('Appendix B', 'W1: pending_content alert fires immediately (no 30-min gate)', 'FAIL', err.message, 'run-6/output/B01-pending-alert-immediate.txt');
  }
}
...
function runBW2StaleRssTest() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix B', 'W2: stale_rss alert covered by test', 'FAIL', 'admin.test.ts not found', 'run-6/output/B02-stale-rss-test.txt');
    return;
  }
  ...
  try {
    ...
    if (hasStaleRssTest && hasStaleRssItBlock) {
      record('Appendix B', 'W2: stale_rss alert covered by test', 'PASS',
        ...
        'run-6/output/B02-stale-rss-test.txt');
    ...
    record('Appendix B', 'W2: stale_rss alert covered by test', 'FAIL', err.message, 'run-6/output/B02-stale-rss-test.txt');
  }
}
...
function runBW3AuthFetchComment() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix B', 'W3: authFetch URL pattern documented', 'FAIL', 'admin/page.tsx not found', 'run-6/output/B03-authfetch-comment.txt');
    return;
  }
  ...
  try {
    ...
    if (hasAuthFetch && (hasExplanationComment || commentLines.length > 0)) {
      record('Appendix B', 'W3: authFetch URL pattern documented', 'PASS',
        ...
        'run-6/output/B03-authfetch-comment.txt');
    ...
    record('Appendix B', 'W3: authFetch URL pattern documented', 'FAIL', err.message, 'run-6/output/B03-authfetch-comment.txt');
  }
}
...
function runBS1AlertKeyStability() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix B', 'S1: Alert key uses type+message (not index)', 'FAIL', 'admin/page.tsx not found', 'run-6/output/B05-alert-key.txt');
    return;
  }
  ...
  try {
    ...
    if (hasTypeMessageKey && !hasIndexKey) {
      record('Appendix B', 'S1: Alert key uses type+message (not index)', 'PASS',
        ...
        'run-6/output/B05-alert-key.txt');
    ...
    record('Appendix B', 'S1: Alert key uses type+message (not index)', 'FAIL', err.message, 'run-6/output/B05-alert-key.txt');
  }
}
...
function runBS3DonutChartCondition() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix B', 'S3: Donut chart shows with partial data (totalPieUsers === 0)', 'FAIL', 'admin/page.tsx not found', 'run-6/output/B06-donut-condition.txt');
    return;
  }
  ...
  try {
    ...
    if (hasTotalPieUsersZeroCheck && !hasPieDataEvery && hasNoSubscriptionData) {
      record('Appendix B', 'S3: Donut chart shows with partial data (totalPieUsers === 0)', 'PASS',
        ...
        'run-6/output/B06-donut-condition.txt');
    ...
    record('Appendix B', 'S3: Donut chart shows with partial data (totalPieUsers === 0)', 'FAIL', err.message, 'run-6/output/B06-donut-condition.txt');
  }
}
...
async function runJobsChecks(adminToken, childToken) {
  ...
  console.log('\n[Jobs] D4: KNOWN_JOBS in apps/api/src/services/job-runner.ts');
  {
    ...
    if (!existsSync(jobRunnerPath)) {
      ...
      record('Jobs', 'D4: job-runner.ts exports KNOWN_JOBS with 10 entries', 'FAIL', 'File not found', 'run-6/output/D04-job-runner-check.txt');
    } else {
      try {
        ...
        if (hasKnownJobs && entryCount === 10) {
          record('Jobs', 'D4: job-runner.ts exports KNOWN_JOBS with 10 entries', 'PASS',
            ...
            'run-6/output/D04-job-runner-check.txt');
        ...
        record('Jobs', 'D4: job-runner.ts exports KNOWN_JOBS with 10 entries', 'FAIL', err.message, 'run-6/output/D04-job-runner-check.txt');
      }
    }
  }
    ...
    try {
      ...
      if (hasJobRunModel && migrationExists) {
        record('Jobs', 'D9: JobRun model in schema.prisma + migration exists', 'PASS',
          ...
          'run-6/output/D09-jobrun-schema-check.txt');
      ...
      record('Jobs', 'D9: JobRun model in schema.prisma + migration exists', 'FAIL', err.message, 'run-6/output/D09-jobrun-schema-check.txt');
    }
  }
  ...
  console.log('\n[Jobs] D10: All 10 job files export run* functions');
  {
    ...
    if (allOk) {
      record('Jobs', 'D10: All 10 job files export run* functions', 'PASS',
        ...
        'run-6/output/D10-job-instrumentation-check.txt');
    } else {
      ...
      try {
        ...
        if (hasJobName && hasConfirmAndRun) {
          record('Jobs', 'D11: Jobs page has jobName + "Confirm & Run"', 'PASS',
            ...
            'run-6/output/D11-jobs-page-check.txt');
        } else {
          const missing = [];
        ...
        record('Jobs', 'D11: Jobs page has jobName + "Confirm & Run"', 'FAIL', err.message, 'run-6/output/D11-jobs-page-check.txt');
      }
    } else {
      saveOutput('D11-jobs-page-check.txt', output);
      record('Jobs', 'D11: Jobs page has jobName + "Confirm & Run"', 'FAIL',
  ...
  console.log('\n[Jobs] D12: Overview page references /admin/jobs');
  {
    ...
    if (!existsSync(overviewPath)) {
      ...
      record('Jobs', 'D12: Overview page has "View all jobs" link to /admin/jobs', 'FAIL',
        ...
        'run-6/output/D12-overview-jobs-panel-check.txt');
    } else {
      try {
        ...
        record('Jobs', 'D12: Overview page has "View all jobs" link to /admin/jobs', 'FAIL', err.message, 'run-6/output/D12-overview-jobs-panel-check.txt');
      }
    }
  }
}
    ...
    try {
      ...
      if (hasConditionalAriaLabel && !hasBadFallback && hasAriaHidden) {
        record('Appendix C', 'C2: Drawer aria-label is undefined when closed', 'PASS',
          ...
          'run-6/output/C2-drawer-aria-label.txt');
      } else {
      ...
      record('Appendix C', 'C2: Drawer aria-label is undefined when closed', 'FAIL', err.message, 'run-6/output/C2-drawer-aria-label.txt');
    }
  }
    ...
    try {
      ...
      if (staticImports.length >= 9 && !hasDynamicImport && missingJobs.length === 0) {
        record('Appendix C', 'C3: job-runner.ts uses static imports for all 10 jobs', 'PASS',
          ...
          'run-6/output/C3-static-imports.txt');
      } else {
      ...
      record('Appendix C', 'C3: job-runner.ts uses static imports for all 10 jobs', 'FAIL', err.message, 'run-6/output/C3-static-imports.txt');
    }
  }
  ...
  console.log('\n[Appendix C] C5: No (prisma as any) casts in admin.ts, job-runner.ts, or jobs/');
  {
    ...
    if (matches.length === 0) {
      record('Appendix C', 'C5: No (prisma as any) casts in admin.ts, job-runner.ts, or jobs/', 'PASS',
        ...
        'run-6/output/C5-prisma-any-check.txt');
    } else {
    ...
    try {
      ...
      if (hasErrorTest) {
        record('Appendix C', 'C6: admin.test.ts has statusLabel ERROR test', 'PASS',
          ...
          'run-6/output/C6-error-test-check.txt');
      } else {
      ...
      record('Appendix C', 'C6: admin.test.ts has statusLabel ERROR test', 'FAIL', err.message, 'run-6/output/C6-error-test-check.txt');
    }
  }
    ...
    try {
      ...
      if (hasSyncLimiterImport && hasSyncLimiterInRoute) {
        record('Appendix C', 'C7: syncLimiter applied to trigger endpoint', 'PASS',
          ...
          'run-6/output/C7-sync-limiter-check.txt');
      } else {
      ...
      record('Appendix C', 'C7: syncLimiter applied to trigger endpoint', 'FAIL', err.message, 'run-6/output/C7-sync-limiter-check.txt');
    }
  }
    ...
    try {
      ...
      if (hasAssertion || hasThrow) {
        record('Appendix C', 'C8: job-runner.ts has JOB_MAP/JOB_FREQUENCIES sync assertion', 'PASS',
          ...
          'run-6/output/C8-sync-assertion-check.txt');
      } else {
      ...
      record('Appendix C', 'C8: job-runner.ts has JOB_MAP/JOB_FREQUENCIES sync assertion', 'FAIL', err.message, 'run-6/output/C8-sync-assertion-check.txt');
    }
  }
    ...
    try {
      ...
      if (hasStatusLabelPolling && !hasOldPolling) {
        record('Appendix C', 'C9: Polling condition uses statusLabel === "RUNNING"', 'PASS',
          ...
          'run-6/output/C9-polling-condition-check.txt');
      } else {
      ...
      record('Appendix C', 'C9: Polling condition uses statusLabel === "RUNNING"', 'FAIL', err.message, 'run-6/output/C9-polling-condition-check.txt');
    }
  }
    ...
    try {
      ...
      if (hasAriaLive) {
        record('Appendix C', 'C10: Jobs table container has aria-live="polite"', 'PASS',
          ...
          'run-6/output/C10-aria-live-check.txt');
      } else {
      ...
      record('Appendix C', 'C10: Jobs table container has aria-live="polite"', 'FAIL', err.message, 'run-6/output/C10-aria-live-check.txt');
    }
  }
...
function generateReport() {
  ...
  const run5Results = {
    ...
    // Section 5: Jobs (all PASS in Run 5)
  ...
  let md = `# Validation Report — Run 6 (post /t-review #3)\n\n`;
  ...
  md += `## Comparison with Run 5\n\n`;
  md += `| Check | Run 5 | Run 6 | Change |\n`;
  md += `|---|---|---|---|\n`;

  const nonAppendixCResults = results.filter((r) => r.section !== 'Appendix C');
  for (const r of nonAppendixCResults) {
    const run5prev = run5Results[r.name] ?? 'N/A';
    const run6 = r.status;
    let change = '—';
    if (run5prev !== 'N/A' && run5prev !== run6) {
      if (run5prev === 'SKIP' && run6 === 'PASS') change = '⬆ Improved';
      else if (run5prev === 'PASS' && run6 === 'FAIL') change = '⬇ Regression';
      else if (run5prev === 'FAIL' && run6 === 'PASS') change = '⬆ Fixed';
      else change = `${run5prev} → ${run6}`;
    }
    md += `| ${r.name} | ${icon(run5prev)} ${run5prev} | ${icon(run6)} ${run6} | ${change} |\n`;
  }

  for (const r of appendixCResults) {
    md += `| ${r.name} | — N/A | ${icon(r.status)} ${r.status} | New in Run 6 |\n`;
  }
...
// ===========================================================================
async function main() {
  console.log('=== Admin Dashboard — Validation Script Run 6 (post /t-review #3) ===\n');
