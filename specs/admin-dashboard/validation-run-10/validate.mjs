/**
 * Admin Dashboard — Validation Script Run 10 (post /t-review #4 (prd5.md Analytics))
 ...
 *
 * Generates report: specs/admin-dashboard/validation-assets/validation-report-run-10.md
 */
...
const RUN_DIR = resolve(ASSETS_DIR, 'run-10');
const API_DIR = resolve(RUN_DIR, 'api');
const OUTPUT_DIR = resolve(RUN_DIR, 'output');
const REPORT_PATH = resolve(ASSETS_DIR, 'validation-report-run-10.md');
...
// ===========================================================================
// SECTION 1: Re-run of original checks (mirrors Run 8 exactly, paths → run-10)
// ===========================================================================
...
async function runA02BatchMixedTypes(adminToken) {
  ...
  if (pendingNews.length === 0 || pendingReels.length === 0) {
    ...
    if (newsOk && reelOk) {
      record('Appendix A', 'C2: Batch validates both news and reel types (400 on missing reason)', 'PASS', 'type=news → 400, type=reel → 400', 'run-10/api/A02-batch-types.json');
    } else {
      record('Appendix A', 'C2: Batch validates both news and reel types (400 on missing reason)', 'FAIL', `news400=${newsOk}, reel400=${reelOk}`, 'run-10/api/A02-batch-types.json');
    }
    return;
  }
  ...
  savePayload('A02-batch-types.json', result);
  if (newsOk && reelOk) {
    record('Appendix A', 'C2: Batch accepts news and reel types independently', 'PASS', 'news: ok, reel: ok', 'run-10/api/A02-batch-types.json');
  } else {
    record('Appendix A', 'C2: Batch accepts news and reel types independently', 'FAIL', `news=${newsOk}, reel=${reelOk}`, 'run-10/api/A02-batch-types.json');
  }
}
...
function runA04ApiBaseDuplicate() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix A', 'W1: No duplicate API_BASE in moderation page', 'FAIL', 'File not found', 'run-10/output/A04-api-base-check.txt');
    return;
  }
  try {
    ...
    saveOutput('A04-api-base-check.txt', output);
    if (matches.length === 0) {
      record('Appendix A', 'W1: No duplicate API_BASE in moderation page', 'PASS', 'const API_BASE not found (uses centralized import)', 'run-10/output/A04-api-base-check.txt');
    } else if (matches.length === 1) {
      record('Appendix A', 'W1: No duplicate API_BASE in moderation page', 'PASS', 'Only 1 const API_BASE declaration', 'run-10/output/A04-api-base-check.txt');
    } else {
      record('Appendix A', 'W1: No duplicate API_BASE in moderation page', 'FAIL', `Found ${matches.length} occurrences`, 'run-10/output/A04-api-base-check.txt');
    }
  } catch (err) {
    saveOutput('A04-api-base-check.txt', `Error: ${err.message}`);
    record('Appendix A', 'W1: No duplicate API_BASE in moderation page', 'FAIL', err.message, 'run-10/output/A04-api-base-check.txt');
  }
}
...
function runA05TableKeyStability() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix A', 'W2: AdminTable does not use index as key', 'FAIL', 'File not found', 'run-10/output/A05-table-key-check.txt');
    return;
  }
  try {
    const content = readFileSync(filePath, 'utf8');
    ...
    if (dataRowBadKeys.length === 0 && hasStableDataKey) {
      record('Appendix A', 'W2: AdminTable does not use index as key', 'PASS', 'Data rows use stable id-based key', 'run-10/output/A05-table-key-check.txt');
    } else {
      record('Appendix A', 'W2: AdminTable does not use index as key', 'FAIL', `dataRowBadKeys=${dataRowBadKeys.length}, stableKey=${hasStableDataKey}`, 'run-10/output/A05-table-key-check.txt');
    }
  } catch (err) {
    saveOutput('A05-table-key-check.txt', `Error: ${err.message}`);
    record('Appendix A', 'W2: AdminTable does not use index as key', 'FAIL', err.message, 'run-10/output/A05-table-key-check.txt');
  }
}
...
function runA07AriaCurrentSidebar() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix A', 'W5: AdminSidebar uses aria-current', 'FAIL', 'File not found', 'run-10/output/A07-aria-current-check.txt');
    return;
  }
  try {
    const content = readFileSync(filePath, 'utf8');
    ...
    saveOutput('A07-aria-current-check.txt', output);
    if (matches.length > 0) {
      record('Appendix A', 'W5: AdminSidebar uses aria-current', 'PASS', `Found ${matches.length} aria-current attribute(s)`, 'run-10/output/A07-aria-current-check.txt');
    } else {
      record('Appendix A', 'W5: AdminSidebar uses aria-current', 'FAIL', 'No aria-current found', 'run-10/output/A07-aria-current-check.txt');
    }
  } catch (err) {
    saveOutput('A07-aria-current-check.txt', `Error: ${err.message}`);
    record('Appendix A', 'W5: AdminSidebar uses aria-current', 'FAIL', err.message, 'run-10/output/A07-aria-current-check.txt');
  }
}
...
function runA08CryptoPassword() {
  ...
  if (!existsSync(scriptPath)) {
    ...
    record('Appendix A', 'S2: Password uses crypto.randomBytes (not Math.random)', 'FAIL', 'Script not found', 'run-10/output/A08-crypto-password-check.txt');
    return;
  }
  try {
    ...
    if (hasCryptoRandomBytes && !hasMathRandom) {
      record('Appendix A', 'S2: Password uses crypto.randomBytes (not Math.random)', 'PASS', 'crypto.randomBytes: ✓, Math.random: ✗', 'run-10/output/A08-crypto-password-check.txt');
    } else if (!hasCryptoRandomBytes) {
      record('Appendix A', 'S2: Password uses crypto.randomBytes (not Math.random)', 'FAIL', 'crypto.randomBytes not found', 'run-10/output/A08-crypto-password-check.txt');
    } else {
      record('Appendix A', 'S2: Password uses crypto.randomBytes (not Math.random)', 'FAIL', 'Math.random still present', 'run-10/output/A08-crypto-password-check.txt');
    }
  } catch (err) {
    saveOutput('A08-crypto-password-check.txt', `Error: ${err.message}`);
    record('Appendix A', 'S2: Password uses crypto.randomBytes (not Math.random)', 'FAIL', err.message, 'run-10/output/A08-crypto-password-check.txt');
  }
}
...
function runA09ErrorState() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix A', 'S5: Moderation page has role="alert" for errors', 'FAIL', 'File not found', 'run-10/output/A09-error-state-check.txt');
    return;
  }
  try {
    const content = readFileSync(filePath, 'utf8');
    ...
    saveOutput('A09-error-state-check.txt', output);
    if (hasRoleAlert) {
      record('Appendix A', 'S5: Moderation page has role="alert" for errors', 'PASS', 'role="alert" found in moderation page', 'run-10/output/A09-error-state-check.txt');
    } else {
      record('Appendix A', 'S5: Moderation page has role="alert" for errors', 'FAIL', 'role="alert" not found', 'run-10/output/A09-error-state-check.txt');
    }
  } catch (err) {
    saveOutput('A09-error-state-check.txt', `Error: ${err.message}`);
    record('Appendix A', 'S5: Moderation page has role="alert" for errors', 'FAIL', err.message, 'run-10/output/A09-error-state-check.txt');
  }
}
...
function runC05AllSystemsOperational() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Part C', 'C5: Overview shows "All systems operational" when alerts=[]', 'FAIL', 'File not found', 'run-10/output/C05-all-systems-check.txt');
    return;
  }
  try {
    const content = readFileSync(filePath, 'utf8');
    ...
    if (hasAlertsLengthCheck && hasAllSystemsText) {
      record('Part C', 'C5: Overview shows "All systems operational" when alerts=[]', 'PASS', 'Condition and text both present in source', 'run-10/output/C05-all-systems-check.txt');
    } else {
      record('Part C', 'C5: Overview shows "All systems operational" when alerts=[]', 'FAIL', `alerts.length check=${hasAlertsLengthCheck}, text=${hasAllSystemsText}`, 'run-10/output/C05-all-systems-check.txt');
    }
  } catch (err) {
    saveOutput('C05-all-systems-check.txt', `Error: ${err.message}`);
    record('Part C', 'C5: Overview shows "All systems operational" when alerts=[]', 'FAIL', err.message, 'run-10/output/C05-all-systems-check.txt');
  }
}
...
function runC06PendingContentAlert() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Part C', 'C6: pending_content alert fires for items >30min old', 'FAIL', 'admin.ts not found', 'run-10/output/C06-pending-alert-check.txt');
    return;
  }
  const content = readFileSync(filePath, 'utf8');
  ...
  if (hasBuildAlerts && has30minCheck && hasPendingType && hasWarningSeverity) {
    record('Part C', 'C6: pending_content alert fires for items >30min old', 'PASS', 'buildAlerts() has 30-min threshold, pending_content type, and warning severity verified in source', 'run-10/output/C06-pending-alert-check.txt');
  } else {
    const missing = [];
    ...
    record('Part C', 'C6: pending_content alert fires for items >30min old', 'FAIL', missing.join(', '), 'run-10/output/C06-pending-alert-check.txt');
  }
}
...
function runC07PendingCriticalSourceCheck() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Part C', 'C7: admin.ts has pendingTotal > 50 → pending_content_critical', 'FAIL', 'File not found', 'run-10/output/C07-pending-critical-check.txt');
    return;
  }
  try {
    const content = readFileSync(filePath, 'utf8');
    ...
    if (hasCriticalCondition && hasCriticalType && hasErrorSeverity) {
      record('Part C', 'C7: admin.ts has pendingTotal > 50 → pending_content_critical', 'PASS', 'Condition, type, and severity all present in source', 'run-10/output/C07-pending-critical-check.txt');
    } else {
      record('Part C', 'C7: admin.ts has pendingTotal > 50 → pending_content_critical', 'FAIL', `condition=${hasCriticalCondition}, type=${hasCriticalType}, severity=${hasErrorSeverity}`, 'run-10/output/C07-pending-critical-check.txt');
    }
  } catch (err) {
    saveOutput('C07-pending-critical-check.txt', `Error: ${err.message}`);
    record('Part C', 'C7: admin.ts has pendingTotal > 50 → pending_content_critical', 'FAIL', err.message, 'run-10/output/C07-pending-critical-check.txt');
  }
}
...
async function runBW1PendingAlertImmediate(adminToken) {
  ...
  if (!existsSync(adminTsPath)) {
    ...
    record('Appendix B', 'W1: pending_content alert fires immediately (no 30-min gate)', 'FAIL', 'admin.ts not found', 'run-10/output/B01-pending-alert-immediate.txt');
    return;
  }
  try {
    ...
    if (hasBuildAlerts && hasPendingTotalCheck && hasPendingContentType && hasNewestJustArrived && hasForOver30Min) {
      record('Appendix B', 'W1: pending_content alert fires immediately (no 30-min gate)', 'PASS',
        ...
        'run-10/output/B01-pending-alert-immediate.txt');
    } else {
      const missing = [];
      if (!hasBuildAlerts) missing.push('buildAlerts() not found');
      ...
      record('Appendix B', 'W1: pending_content alert fires immediately (no 30-min gate)', 'FAIL', missing.join(', '), 'run-10/output/B01-pending-alert-immediate.txt');
    }
  } catch (err) {
    saveOutput('B01-pending-alert-immediate.txt', `Error: ${err.message}`);
    record('Appendix B', 'W1: pending_content alert fires immediately (no 30-min gate)', 'FAIL', err.message, 'run-10/output/B01-pending-alert-immediate.txt');
  }
}
...
function runBW2StaleRssTest() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix B', 'W2: stale_rss alert covered by test', 'FAIL', 'admin.test.ts not found', 'run-10/output/B02-stale-rss-test.txt');
    return;
  }
  try {
    const content = readFileSync(filePath, 'utf8');
    ...
    if (hasStaleRssTest && hasStaleRssItBlock) {
      record('Appendix B', 'W2: stale_rss alert covered by test', 'PASS', 'stale_rss test found in admin.test.ts', 'run-10/output/B02-stale-rss-test.txt');
    } else if (hasStaleRssTest) {
      record('Appendix B', 'W2: stale_rss alert covered by test', 'PASS', '"stale_rss" referenced in test file', 'run-10/output/B02-stale-rss-test.txt');
    } else {
      record('Appendix B', 'W2: stale_rss alert covered by test', 'FAIL', '"stale_rss" not found in admin.test.ts', 'run-10/output/B02-stale-rss-test.txt');
    }
  } catch (err) {
    saveOutput('B02-stale-rss-test.txt', `Error: ${err.message}`);
    record('Appendix B', 'W2: stale_rss alert covered by test', 'FAIL', err.message, 'run-10/output/B02-stale-rss-test.txt');
  }
}
...
function runBW3AuthFetchComment() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix B', 'W3: authFetch URL pattern documented', 'FAIL', 'admin/page.tsx not found', 'run-10/output/B03-authfetch-comment.txt');
    return;
  }
  try {
    const content = readFileSync(filePath, 'utf8');
    ...
    if (hasAuthFetch && (hasExplanationComment || commentLines.length > 0)) {
      record('Appendix B', 'W3: authFetch URL pattern documented', 'PASS', `Comment explains why full URLs are used with API_BASE`, 'run-10/output/B03-authfetch-comment.txt');
    } else if (!hasAuthFetch) {
      record('Appendix B', 'W3: authFetch URL pattern documented', 'FAIL', 'authFetch not found in admin/page.tsx', 'run-10/output/B03-authfetch-comment.txt');
    } else {
      record('Appendix B', 'W3: authFetch URL pattern documented', 'FAIL', 'No comment found explaining why full URLs are used with authFetch', 'run-10/output/B03-authfetch-comment.txt');
    }
  } catch (err) {
    saveOutput('B03-authfetch-comment.txt', `Error: ${err.message}`);
    record('Appendix B', 'W3: authFetch URL pattern documented', 'FAIL', err.message, 'run-10/output/B03-authfetch-comment.txt');
  }
}
...
function runBS1AlertKeyStability() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix B', 'S1: Alert key uses type+message (not index)', 'FAIL', 'admin/page.tsx not found', 'run-10/output/B05-alert-key.txt');
    return;
  }
  try {
    const content = readFileSync(filePath, 'utf8');
    ...
    if (hasTypeMessageKey && !hasIndexKey) {
      record('Appendix B', 'S1: Alert key uses type+message (not index)', 'PASS', 'key uses type+message; no index-based key', 'run-10/output/B05-alert-key.txt');
    } else if (!hasTypeMessageKey) {
      record('Appendix B', 'S1: Alert key uses type+message (not index)', 'FAIL', 'type+message key pattern not found in alerts .map()', 'run-10/output/B05-alert-key.txt');
    } else {
      record('Appendix B', 'S1: Alert key uses type+message (not index)', 'FAIL', 'Index-based key detected in alerts .map()', 'run-10/output/B05-alert-key.txt');
    }
  } catch (err) {
    saveOutput('B05-alert-key.txt', `Error: ${err.message}`);
    record('Appendix B', 'S1: Alert key uses type+message (not index)', 'FAIL', err.message, 'run-10/output/B05-alert-key.txt');
  }
}
...
function runBS3DonutChartCondition() {
  ...
  if (!existsSync(filePath)) {
    ...
    record('Appendix B', 'S3: Donut chart shows with partial data (totalPieUsers === 0)', 'FAIL', 'admin/page.tsx not found', 'run-10/output/B06-donut-condition.txt');
    return;
  }
  try {
    const content = readFileSync(filePath, 'utf8');
    ...
    if (hasTotalPieUsersZeroCheck && !hasPieDataEvery && hasNoSubscriptionData) {
      record('Appendix B', 'S3: Donut chart shows with partial data (totalPieUsers === 0)', 'PASS', 'Empty-state uses totalPieUsers === 0; pieData.every removed', 'run-10/output/B06-donut-condition.txt');
    } else if (hasPieDataEvery) {
      record('Appendix B', 'S3: Donut chart shows with partial data (totalPieUsers === 0)', 'FAIL', 'pieData.every still present', 'run-10/output/B06-donut-condition.txt');
    } else if (!hasTotalPieUsersZeroCheck) {
      record('Appendix B', 'S3: Donut chart shows with partial data (totalPieUsers === 0)', 'FAIL', 'totalPieUsers === 0 condition not found', 'run-10/output/B06-donut-condition.txt');
    } else {
      record('Appendix B', 'S3: Donut chart shows with partial data (totalPieUsers === 0)', 'FAIL', `totalPieUsers check=${hasTotalPieUsersZeroCheck}, noSubData=${hasNoSubscriptionData}`, 'run-10/output/B06-donut-condition.txt');
    }
  } catch (err) {
    saveOutput('B06-donut-condition.txt', `Error: ${err.message}`);
    record('Appendix B', 'S3: Donut chart shows with partial data (totalPieUsers === 0)', 'FAIL', err.message, 'run-10/output/B06-donut-condition.txt');
  }
}
...
async function runJobsChecks(adminToken, childToken) {
  ...
  // D3 — returns jobs with correct shape (>=10 with compute-analytics added in run-10)
  ...
  console.log('\n[Jobs] D4: KNOWN_JOBS in apps/api/src/services/job-runner.ts');
  {
    ...
    if (!existsSync(jobRunnerPath)) {
      ...
      record('Jobs', 'D4: job-runner.ts exports KNOWN_JOBS with 10 entries', 'FAIL', 'File not found', 'run-10/output/D04-job-runner-check.txt');
    } else {
      try {
        ...
        if (hasKnownJobs && entryCount >= 10) {
          record('Jobs', 'D4: job-runner.ts exports KNOWN_JOBS with 10 entries', 'PASS', `KNOWN_JOBS found, JOB_FREQUENCIES has ${entryCount} entries`, 'run-10/output/D04-job-runner-check.txt');
        } else if (!hasKnownJobs) {
          record('Jobs', 'D4: job-runner.ts exports KNOWN_JOBS with 10 entries', 'FAIL', 'KNOWN_JOBS not found in job-runner.ts', 'run-10/output/D04-job-runner-check.txt');
        } else {
          record('Jobs', 'D4: job-runner.ts exports KNOWN_JOBS with 10 entries', 'FAIL', `Expected ≥10 entries in JOB_FREQUENCIES, found ${entryCount}`, 'run-10/output/D04-job-runner-check.txt');
        }
      } catch (err) {
        saveOutput('D04-job-runner-check.txt', `Error: ${err.message}`);
        record('Jobs', 'D4: job-runner.ts exports KNOWN_JOBS with 10 entries', 'FAIL', err.message, 'run-10/output/D04-job-runner-check.txt');
      }
    }
  }
  ...
  console.log('\n[Jobs] D9: JobRun model in schema.prisma + migration file');
  {
    ...
    try {
      ...
      if (hasJobRunModel && migrationExists) {
        record('Jobs', 'D9: JobRun model in schema.prisma + migration exists', 'PASS', 'model JobRun found; migration file present', 'run-10/output/D09-jobrun-schema-check.txt');
      } else {
        record('Jobs', 'D9: JobRun model in schema.prisma + migration exists', 'FAIL', `JobRun=${hasJobRunModel}, migration=${migrationExists}`, 'run-10/output/D09-jobrun-schema-check.txt');
      }
    } catch (err) {
      saveOutput('D09-jobrun-schema-check.txt', `Error: ${err.message}`);
      record('Jobs', 'D9: JobRun model in schema.prisma + migration exists', 'FAIL', err.message, 'run-10/output/D09-jobrun-schema-check.txt');
    }
  }
  ...
  console.log('\n[Jobs] D10: All 10 job files export run* functions');
  {
    ...
    if (allOk) {
      record('Jobs', 'D10: All 10 job files export run* functions', 'PASS', 'All 10 job files exist and export their run* functions', 'run-10/output/D10-job-instrumentation-check.txt');
    } else {
      const failed = results10.filter((r) => r.status !== 'OK').map((r) => `${r.fn}(${r.status})`).join(', ');
      record('Jobs', 'D10: All 10 job files export run* functions', 'FAIL', `${failedCount} job(s) failed: ${failed}`, 'run-10/output/D10-job-instrumentation-check.txt');
    }
  }
    ...
    if (exists) {
      try {
        ...
        if (hasJobName && hasConfirmAndRun) {
          record('Jobs', 'D11: Jobs page has jobName + "Confirm & Run"', 'PASS', 'jobs/page.tsx exists with jobName and "Confirm & Run"', 'run-10/output/D11-jobs-page-check.txt');
        } else {
          const missing = [];
          if (!hasJobName) missing.push('jobName not found');
          if (!hasConfirmAndRun) missing.push('"Confirm & Run" not found');
          record('Jobs', 'D11: Jobs page has jobName + "Confirm & Run"', 'FAIL', missing.join(', '), 'run-10/output/D11-jobs-page-check.txt');
        }
      } catch (err) {
        saveOutput('D11-jobs-page-check.txt', `Error reading file: ${err.message}`);
        record('Jobs', 'D11: Jobs page has jobName + "Confirm & Run"', 'FAIL', err.message, 'run-10/output/D11-jobs-page-check.txt');
      }
    } else {
      saveOutput('D11-jobs-page-check.txt', output);
      record('Jobs', 'D11: Jobs page has jobName + "Confirm & Run"', 'FAIL', 'apps/web/src/app/(admin)/admin/jobs/page.tsx not found', 'run-10/output/D11-jobs-page-check.txt');
    }
  }
  ...
  console.log('\n[Jobs] D12: Overview page references /admin/jobs');
  {
    ...
    if (!existsSync(overviewPath)) {
      ...
      record('Jobs', 'D12: Overview page has "View all jobs" link to /admin/jobs', 'FAIL', 'apps/web/src/app/(admin)/admin/page.tsx not found', 'run-10/output/D12-overview-jobs-panel-check.txt');
    } else {
      try {
        ...
        saveOutput('D12-overview-jobs-panel-check.txt', output);
        if (hasJobsLink) {
          record('Jobs', 'D12: Overview page has "View all jobs" link to /admin/jobs', 'PASS', '/admin/jobs reference found', 'run-10/output/D12-overview-jobs-panel-check.txt');
        } else {
          record('Jobs', 'D12: Overview page has "View all jobs" link to /admin/jobs', 'FAIL', '/admin/jobs not referenced in admin/page.tsx', 'run-10/output/D12-overview-jobs-panel-check.txt');
        }
      } catch (err) {
        saveOutput('D12-overview-jobs-panel-check.txt', `Error: ${err.message}`);
        record('Jobs', 'D12: Overview page has "View all jobs" link to /admin/jobs', 'FAIL', err.message, 'run-10/output/D12-overview-jobs-panel-check.txt');
      }
    }
  }
}
...
async function runAppendixCChecks(adminToken) {
  ...
  console.log('\n[Appendix C] C2: Drawer aria-label is undefined when closed');
  {
    ...
    try {
      ...
      if (hasConditionalAriaLabel && !hasBadFallback && hasAriaHidden) {
        record('Appendix C', 'C2: Drawer aria-label is undefined when closed', 'PASS', 'aria-label uses ternary with undefined fallback; aria-hidden={!isOpen} present', 'run-10/output/C2-drawer-aria-label.txt');
      } else {
        record('Appendix C', 'C2: Drawer aria-label is undefined when closed', 'FAIL', `conditional=${hasConditionalAriaLabel}, badFallback=${hasBadFallback}, ariaHidden=${hasAriaHidden}`, 'run-10/output/C2-drawer-aria-label.txt');
      }
    } catch (err) {
      saveOutput('C2-drawer-aria-label.txt', `Error: ${err.message}`);
      record('Appendix C', 'C2: Drawer aria-label is undefined when closed', 'FAIL', err.message, 'run-10/output/C2-drawer-aria-label.txt');
    }
  }
  ...
  console.log('\n[Appendix C] C3: job-runner.ts uses static imports for all jobs');
  {
    ...
    try {
      ...
      if (staticImports.length >= 9 && !hasDynamicImport && missingJobs.length === 0) {
        record('Appendix C', 'C3: job-runner.ts uses static imports for all 10 jobs', 'PASS', `${staticImports.length} static job imports, no dynamic import()`, 'run-10/output/C3-static-imports.txt');
      } else {
        record('Appendix C', 'C3: job-runner.ts uses static imports for all 10 jobs', 'FAIL', `staticImports=${staticImports.length}, dynamicImport=${hasDynamicImport}, missing=[${missingJobs.join(',')}]`, 'run-10/output/C3-static-imports.txt');
      }
    } catch (err) {
      saveOutput('C3-static-imports.txt', `Error: ${err.message}`);
      record('Appendix C', 'C3: job-runner.ts uses static imports for all 10 jobs', 'FAIL', err.message, 'run-10/output/C3-static-imports.txt');
    }
  }
  ...
  console.log('\n[Appendix C] C5: No (prisma as any) casts in admin.ts, job-runner.ts, or jobs/');
  {
    ...
    if (matches.length === 0) {
      record('Appendix C', 'C5: No (prisma as any) casts in admin.ts, job-runner.ts, or jobs/', 'PASS', `${filesToCheck.length} files checked, 0 matches`, 'run-10/output/C5-prisma-any-check.txt');
    } else {
      record('Appendix C', 'C5: No (prisma as any) casts in admin.ts, job-runner.ts, or jobs/', 'FAIL', `Found ${matches.length} cast(s)`, 'run-10/output/C5-prisma-any-check.txt');
    }
  }
  ...
  console.log('\n[Appendix C] C6: admin.test.ts has statusLabel ERROR test');
  {
    ...
    try {
      ...
      if (hasErrorTest) {
        record('Appendix C', 'C6: admin.test.ts has statusLabel ERROR test', 'PASS', 'Found statusLabel ERROR test', 'run-10/output/C6-error-test-check.txt');
      } else {
        record('Appendix C', 'C6: admin.test.ts has statusLabel ERROR test', 'FAIL', 'No test found for statusLabel ERROR', 'run-10/output/C6-error-test-check.txt');
      }
    } catch (err) {
      saveOutput('C6-error-test-check.txt', `Error: ${err.message}`);
      record('Appendix C', 'C6: admin.test.ts has statusLabel ERROR test', 'FAIL', err.message, 'run-10/output/C6-error-test-check.txt');
    }
  }
  ...
  console.log('\n[Appendix C] C7: syncLimiter applied to trigger endpoint');
  {
    ...
    try {
      ...
      if (hasSyncLimiterImport && hasSyncLimiterInRoute) {
        record('Appendix C', 'C7: syncLimiter applied to trigger endpoint', 'PASS', 'syncLimiter imported and used in /jobs/:name/trigger route', 'run-10/output/C7-sync-limiter-check.txt');
      } else {
        record('Appendix C', 'C7: syncLimiter applied to trigger endpoint', 'FAIL', `imported=${hasSyncLimiterImport}, inRoute=${hasSyncLimiterInRoute}`, 'run-10/output/C7-sync-limiter-check.txt');
      }
    } catch (err) {
      saveOutput('C7-sync-limiter-check.txt', `Error: ${err.message}`);
      record('Appendix C', 'C7: syncLimiter applied to trigger endpoint', 'FAIL', err.message, 'run-10/output/C7-sync-limiter-check.txt');
    }
  }
  ...
  console.log('\n[Appendix C] C8: job-runner.ts has JOB_MAP/JOB_FREQUENCIES sync assertion');
  {
    ...
    try {
      ...
      if (hasAssertion || hasThrow) {
        record('Appendix C', 'C8: job-runner.ts has JOB_MAP/JOB_FREQUENCIES sync assertion', 'PASS', 'Startup sync assertion found', 'run-10/output/C8-sync-assertion-check.txt');
      } else {
        record('Appendix C', 'C8: job-runner.ts has JOB_MAP/JOB_FREQUENCIES sync assertion', 'FAIL', 'No sync assertion found', 'run-10/output/C8-sync-assertion-check.txt');
      }
    } catch (err) {
      saveOutput('C8-sync-assertion-check.txt', `Error: ${err.message}`);
      record('Appendix C', 'C8: job-runner.ts has JOB_MAP/JOB_FREQUENCIES sync assertion', 'FAIL', err.message, 'run-10/output/C8-sync-assertion-check.txt');
    }
  }
  ...
  console.log('\n[Appendix C] C9: Polling condition uses statusLabel === "RUNNING"');
  {
    ...
    try {
      ...
      if (hasStatusLabelPolling && !hasOldPolling) {
        record('Appendix C', 'C9: Polling condition uses statusLabel === "RUNNING"', 'PASS', 'statusLabel polling found, old lastRun?.status polling removed', 'run-10/output/C9-polling-condition-check.txt');
      } else {
        record('Appendix C', 'C9: Polling condition uses statusLabel === "RUNNING"', 'FAIL', `statusLabelPolling=${hasStatusLabelPolling}, oldPolling=${hasOldPolling}`, 'run-10/output/C9-polling-condition-check.txt');
      }
    } catch (err) {
      saveOutput('C9-polling-condition-check.txt', `Error: ${err.message}`);
      record('Appendix C', 'C9: Polling condition uses statusLabel === "RUNNING"', 'FAIL', err.message, 'run-10/output/C9-polling-condition-check.txt');
    }
  }
  ...
  console.log('\n[Appendix C] C10: Jobs table container has aria-live="polite"');
  {
    ...
    try {
      ...
      if (hasAriaLive) {
        record('Appendix C', 'C10: Jobs table container has aria-live="polite"', 'PASS', `aria-live="polite" found`, 'run-10/output/C10-aria-live-check.txt');
      } else {
        record('Appendix C', 'C10: Jobs table container has aria-live="polite"', 'FAIL', 'aria-live="polite" not found in jobs page', 'run-10/output/C10-aria-live-check.txt');
      }
    } catch (err) {
      saveOutput('C10-aria-live-check.txt', `Error: ${err.message}`);
      record('Appendix C', 'C10: Jobs table container has aria-live="polite"', 'FAIL', err.message, 'run-10/output/C10-aria-live-check.txt');
    }
  }
...
async function runAppendixDChecks(adminToken) {
  const API = 'http://localhost:3001';
  const savePayloadD = (filename, data) => { writeFileSync(`${API_DIR}/${filename}`, JSON.stringify(data, null, 2), 'utf8'); return `run-10/api/${filename}`; };
  const saveOutputD = (filename, data) => { writeFileSync(`${OUTPUT_DIR}/${filename}`, data, 'utf8'); return `run-10/output/${filename}`; };
...
async function runAnalyticsChecks(adminToken, childToken) {
  ...
  console.log('\n[Analytics] S9-1: AdminSidebar has /admin/analytics link');
  {
    ...
    if (!existsSync(sidebarPath)) {
      ...
      record('Analytics', 'S9-1: AdminSidebar has /admin/analytics link', 'FAIL', 'AdminSidebar.tsx not found', 'run-10/output/S9-01-sidebar-analytics-link.txt');
    } else {
      try {
        const content = readFileSync(sidebarPath, 'utf8');
        ...
        saveOutput('S9-01-sidebar-analytics-link.txt', output);
        record('Analytics', 'S9-1: AdminSidebar has /admin/analytics link', hasAnalyticsLink ? 'PASS' : 'FAIL', hasAnalyticsLink ? '/admin/analytics link found in AdminSidebar' : '/admin/analytics not found in AdminSidebar.tsx', 'run-10/output/S9-01-sidebar-analytics-link.txt');
      } catch (e) {
        saveOutput('S9-01-sidebar-analytics-link.txt', `Error: ${e.message}`);
        record('Analytics', 'S9-1: AdminSidebar has /admin/analytics link', 'FAIL', e.message, 'run-10/output/S9-01-sidebar-analytics-link.txt');
      }
    }
  }
  ...
  console.log('\n[Analytics] S9-2: analytics/page.tsx exists and has date range buttons');
  {
    ...
    if (!exists) {
      ...
      record('Analytics', 'S9-2: analytics/page.tsx has date range buttons', 'FAIL', 'File not found', 'run-10/output/S9-02-analytics-page-date-range.txt');
    } else {
      try {
        ...
        saveOutput('S9-02-analytics-page-date-range.txt', output);
        if (allFound) {
          record('Analytics', 'S9-2: analytics/page.tsx has date range buttons', 'PASS', hasDynamic ? 'Dynamic date range buttons found (Last {r} days with [7,30,90])' : 'All 3 date range buttons found', 'run-10/output/S9-02-analytics-page-date-range.txt');
        } else {
          const missing = [!has7days && '"Last 7 days"', !has30days && '"Last 30 days"', !has90days && '"Last 90 days"'].filter(Boolean);
          record('Analytics', 'S9-2: analytics/page.tsx has date range buttons', 'FAIL', `Missing: ${missing.join(', ')}`, 'run-10/output/S9-02-analytics-page-date-range.txt');
        }
      } catch (e) {
        saveOutput('S9-02-analytics-page-date-range.txt', `Error: ${e.message}`);
        record('Analytics', 'S9-2: analytics/page.tsx has date range buttons', 'FAIL', e.message, 'run-10/output/S9-02-analytics-page-date-range.txt');
      }
    }
  }
  ...
  console.log('\n[Analytics] S9-3: analytics/page.tsx has data notice');
  {
    ...
    record('Analytics', 'S9-3: analytics/page.tsx has data notice', found ? 'PASS' : 'FAIL', found ? 'Data notice found' : '"yesterday 2am UTC" not found in analytics page', 'run-10/output/S9-03-analytics-data-notice.txt');
  }
  ...
  console.log('\n[Analytics] S9-4: analytics/page.tsx has empty state message');
  {
    ...
    record('Analytics', 'S9-4: analytics/page.tsx has empty state message', found ? 'PASS' : 'FAIL', found ? 'Empty state message found' : '"compute-analytics job runs daily" not found', 'run-10/output/S9-04-analytics-empty-state.txt');
  }
  ...
  console.log('\n[Analytics] S9-5: analytics/page.tsx has DAU/MAU AreaChart');
  {
    ...
    record('Analytics', 'S9-5: analytics/page.tsx has DAU/MAU AreaChart', found ? 'PASS' : 'FAIL', found ? 'AreaChart found in analytics page' : 'AreaChart not found', 'run-10/output/S9-05-analytics-areachart.txt');
  }
  ...
  console.log('\n[Analytics] S9-6: analytics/page.tsx has Retention BarChart with D1/D7');
  {
    ...
    record('Analytics', 'S9-6: analytics/page.tsx has Retention BarChart with D1/D7', found ? 'PASS' : 'FAIL', found ? 'D1/D7 retention metrics found' : 'D1/D7 retention not found in analytics page', 'run-10/output/S9-06-analytics-retention.txt');
  }
  ...
  console.log('\n[Analytics] S9-7: analytics/page.tsx has sport activity chart');
  {
    ...
    record('Analytics', 'S9-7: analytics/page.tsx has sport activity chart', found ? 'PASS' : 'FAIL', found ? 'Sport activity chart found' : 'sport_activity/sportActivity not found in analytics page', 'run-10/output/S9-07-analytics-sport-activity.txt');
  }
  ...
  console.log('\n[Analytics] S9-8: analytics/page.tsx has subscription donut (PieChart + innerRadius)');
  {
    ...
    record('Analytics', 'S9-8: analytics/page.tsx has subscription donut (PieChart + innerRadius)', found ? 'PASS' : 'FAIL', found ? 'innerRadius (donut chart) found' : 'innerRadius not found in analytics page', 'run-10/output/S9-08-analytics-subscription-donut.txt');
  }
  ...
  console.log('\n[Analytics] S9-9: analytics/page.tsx has missions pie chart');
  {
    ...
    record('Analytics', 'S9-9: analytics/page.tsx has missions pie chart', found ? 'PASS' : 'FAIL', found ? 'Missions metrics found in analytics page' : 'missions_completed/missionsClaimed not found', 'run-10/output/S9-09-analytics-missions.txt');
  }
  ...
  console.log('\n[Analytics] S9-10: analytics/page.tsx has parental_activation_rate metric card with MiniProgressBar');
  {
    ...
    record('Analytics', 'S9-10: analytics/page.tsx has parental_activation_rate metric card with MiniProgressBar', found ? 'PASS' : 'FAIL', `parental: ${hasParental}, progressBar: ${hasMiniProgressBar}`, 'run-10/output/S9-10-analytics-parental-activation.txt');
  }
  ...
  console.log('\n[Analytics] S9-11: analytics/page.tsx has consent_rate metric card');
  {
    ...
    record('Analytics', 'S9-11: analytics/page.tsx has consent_rate metric card', found ? 'PASS' : 'FAIL', found ? 'consent_rate found in analytics page' : 'consent_rate/consent not found', 'run-10/output/S9-11-analytics-consent-rate.txt');
  }
  ...
  console.log('\n[Analytics] S9-12: analytics/page.tsx has quiz_engagement metric card');
  {
    ...
    record('Analytics', 'S9-12: analytics/page.tsx has quiz_engagement metric card', found ? 'PASS' : 'FAIL', found ? 'quiz_engagement found' : 'quiz_engagement/Quiz Engagement not found', 'run-10/output/S9-12-analytics-quiz-engagement.txt');
  }
  ...
  console.log('\n[Analytics] S9-13: analytics/page.tsx has top content AdminTable');
  {
    ...
    record('Analytics', 'S9-13: analytics/page.tsx has top content AdminTable', found ? 'PASS' : 'FAIL', found ? 'AdminTable/topContent found' : 'AdminTable/top-content not found', 'run-10/output/S9-13-analytics-top-content.txt');
  }
  ...
  console.log('\n[Analytics] S9-19: AnalyticsSnapshot model in schema.prisma');
  {
    ...
    if (!existsSync(schemaPath)) {
      ...
      record('Analytics', 'S9-19: AnalyticsSnapshot model in schema.prisma', 'FAIL', 'schema.prisma not found', 'run-10/output/S9-19-analytics-snapshot-schema.txt');
    } else {
      const schemaContent = readFileSync(schemaPath, 'utf8');
      const found = /AnalyticsSnapshot/.test(schemaContent);
      output = `AnalyticsSnapshot in schema.prisma: ${found}\n`;
      saveOutput('S9-19-analytics-snapshot-schema.txt', output);
      record('Analytics', 'S9-19: AnalyticsSnapshot model in schema.prisma', found ? 'PASS' : 'FAIL', found ? 'AnalyticsSnapshot model found in schema.prisma' : 'AnalyticsSnapshot not found in schema.prisma', 'run-10/output/S9-19-analytics-snapshot-schema.txt');
    }
  }
  ...
  console.log('\n[Analytics] S9-20: admin-stats.ts exports all 11 compute functions');
  {
    ...
    if (!existsSync(adminStatsPath)) {
      ...
      record('Analytics', 'S9-20: admin-stats.ts exports all 11 compute functions', 'FAIL', 'admin-stats.ts not found', 'run-10/output/S9-20-admin-stats-functions.txt');
    } else {
      ...
      saveOutput('S9-20-admin-stats-functions.txt', output);
      if (missing.length === 0) {
        record('Analytics', 'S9-20: admin-stats.ts exports all 11 compute functions', 'PASS', 'All 11 compute functions found in admin-stats.ts', 'run-10/output/S9-20-admin-stats-functions.txt');
      } else {
        record('Analytics', 'S9-20: admin-stats.ts exports all 11 compute functions', 'FAIL', `Missing: ${missing.join(', ')}`, 'run-10/output/S9-20-admin-stats-functions.txt');
      }
    }
  }
  ...
  console.log('\n[Analytics] S9-21: compute-analytics.ts exists and exports runComputeAnalytics');
  {
    ...
    if (!existsSync(computeAnalyticsPath)) {
      ...
      record('Analytics', 'S9-21: compute-analytics.ts exists and exports runComputeAnalytics', 'FAIL', 'compute-analytics.ts not found', 'run-10/output/S9-21-compute-analytics-exists.txt');
    } else {
      ...
      saveOutput('S9-21-compute-analytics-exists.txt', output);
      record('Analytics', 'S9-21: compute-analytics.ts exists and exports runComputeAnalytics', found ? 'PASS' : 'FAIL', found ? 'runComputeAnalytics found in compute-analytics.ts' : 'runComputeAnalytics not found', 'run-10/output/S9-21-compute-analytics-exists.txt');
    }
  }
  ...
  console.log('\n[Analytics] S9-22: compute-analytics scheduled at 0 2 * * *');
  {
    ...
    record('Analytics', 'S9-22: compute-analytics scheduled at 0 2 * * * and registered in index.ts', pass ? 'PASS' : 'FAIL', pass ? '0 2 * * * in compute-analytics.ts; startComputeAnalyticsJob in index.ts' : `cron=${hasCronSchedule}, index=${hasIndexImport}`, 'run-10/output/S9-22-compute-analytics-cron.txt');
  }
  ...
  console.log('\n[Analytics] S9-23: compute-analytics in KNOWN_JOBS and JOB_FREQUENCIES');
  {
    ...
    if (!existsSync(jobRunnerPath)) {
      ...
      record('Analytics', 'S9-23: compute-analytics in KNOWN_JOBS and JOB_FREQUENCIES', 'FAIL', 'job-runner.ts not found', 'run-10/output/S9-23-compute-analytics-job-runner.txt');
    } else {
      const content = readFileSync(jobRunnerPath, 'utf8');
      ...
      record('Analytics', 'S9-23: compute-analytics in KNOWN_JOBS and JOB_FREQUENCIES', occurrences >= 2 ? 'PASS' : 'FAIL', `Found ${occurrences} occurrence(s) of "compute-analytics" in job-runner.ts (expected ≥2)`, 'run-10/output/S9-23-compute-analytics-job-runner.txt');
    }
  }
  ...
  console.log('\n[Analytics] S9-25: analytics/page.tsx has error state with role="alert"');
  {
    ...
    record('Analytics', 'S9-25: analytics/page.tsx has error state with role="alert"', found ? 'PASS' : 'FAIL', found ? 'role="alert" found in analytics page' : 'role="alert" not found in analytics page', 'run-10/output/S9-25-analytics-error-state.txt');
  }
}
