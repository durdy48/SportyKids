...
check('D02', '/api/auth/upgrade has IDOR protection', () => {
  ...
  assert(upgradeSection.includes('req.auth') && upgradeSection.includes('403'),
    'Upgrade route does not check auth with 403');
  assert(upgradeSection.includes('userId') && upgradeSection.includes('req.auth.userId'),
    'Does not compare userId with req.auth.userId');
  return 'IDOR protection: authenticated users can only upgrade their own account';
});
...
check('D06', 'JWT_SECRET warns in production', () => {
  const content = readFile('apps/api/src/services/auth-service.ts');
  assert(content.includes('production') && content.includes('WARNING'),
    'No production warning for missing JWT_SECRET');
  return 'Logs warning if JWT_SECRET not set in production';
});
