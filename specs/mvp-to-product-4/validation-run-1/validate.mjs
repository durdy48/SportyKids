...
const EVIDENCE_DIR = path.join(ROOT, 'specs/mvp-to-product-4/validation-assets/run-1/output');
const REPORT_PATH = path.join(ROOT, 'specs/mvp-to-product-4/validation-assets/validation-report-run-1.md');
...
function validatePlaywright() {
  ...
  // 4.2 Run Playwright tests — SKIP unless API+web running
  {
    ...
    if (apiRunning && webRunning) {
      ...
      const result = runCommand('cd apps/web && npx playwright test --reporter=list', 180_000);
...
function generateReport() {
  ...
  lines.push('---');
  lines.push(`Evidence files saved to: \`specs/mvp-to-product-4/validation-assets/run-1/output/\``);
