...
const RUN = 2;
const EVIDENCE_DIR = resolve(__dirname, '..', 'validation-assets', `run-${RUN}`);
...
function saveOutput(filename, content) {
  ...
  return `run-${RUN}/output/${filename}`;
}
...
// ---------------------------------------------------------------------------
console.log('\n=== Appendix A: Review Fixes ===');

// 22. EN keywords restored with key PRD terms
try {
  const enMeta = JSON.parse(readProjectFile('apps/mobile/store-metadata/en.json'));
  const kw = enMeta.keywords;
  const hasSafe = kw.includes('safe news for kids');
  const hasReels = kw.includes('sports reels');
  const under100 = kw.length <= 100;
  record(22, 'EN keywords restored', hasSafe && hasReels && under100 ? 'PASS' : 'FAIL',
    `"safe news for kids": ${hasSafe}, "sports reels": ${hasReels}, length: ${kw.length}/100`);
} catch (e) {
  record(22, 'EN keywords restored', 'FAIL', e.message);
}

// 23. ES keywords restored with key PRD terms
try {
  const esMeta = JSON.parse(readProjectFile('apps/mobile/store-metadata/es.json'));
  const kw = esMeta.keywords;
  const hasSafe = kw.includes('noticias seguras');
  const hasReels = kw.includes('reels');
  const under100 = kw.length <= 100;
  record(23, 'ES keywords restored', hasSafe && hasReels && under100 ? 'PASS' : 'FAIL',
    `"noticias seguras": ${hasSafe}, "reels": ${hasReels}, length: ${kw.length}/100`);
} catch (e) {
  record(23, 'ES keywords restored', 'FAIL', e.message);
}

// 24. Dockerfile COPY comments
try {
  const df = readProjectFile('apps/api/Dockerfile');
  const hasComment = df.includes('Workspace node_modules may not exist when npm hoists');
  record(24, 'Dockerfile COPY comments', hasComment ? 'PASS' : 'FAIL',
    hasComment ? 'Comment explains hoisting fallback' : 'Missing explanation comment');
} catch (e) {
  record(24, 'Dockerfile COPY comments', 'FAIL', e.message);
}

// 25. Dockerfile uses npm workspace build scripts
try {
  const df = readProjectFile('apps/api/Dockerfile');
  const hasWorkspaceBuild = df.includes('npm run build --workspace=packages/shared');
  const hasApiBuild = df.includes('npm run build:api');
  record(25, 'Dockerfile workspace builds', hasWorkspaceBuild && hasApiBuild ? 'PASS' : 'FAIL',
    `workspace build: ${hasWorkspaceBuild}, build:api: ${hasApiBuild}`);
} catch (e) {
  record(25, 'Dockerfile workspace builds', 'FAIL', e.message);
}

// 26. Dockerfile runner copies only shared/dist
try {
  const df = readProjectFile('apps/api/Dockerfile');
  const copiesDistOnly = df.includes('packages/shared/dist') && df.includes('packages/shared/package.json');
  // Should NOT copy full packages/shared in runner
  const lines = df.split('\n');
  const runnerStart = lines.findIndex(l => l.includes('AS runner'));
  const runnerLines = lines.slice(runnerStart);
  const copiesFullShared = runnerLines.some(l => l.includes('COPY') && l.includes('/app/packages/shared ') && !l.includes('/dist') && !l.includes('/package.json'));
  record(26, 'Dockerfile shared dist-only', copiesDistOnly && !copiesFullShared ? 'PASS' : 'FAIL',
    `dist copy: ${copiesDistOnly}, no full source: ${!copiesFullShared}`);
} catch (e) {
  record(26, 'Dockerfile shared dist-only', 'FAIL', e.message);
}

// 27. config.ts type cast comment
try {
  const config = readProjectFile('apps/mobile/src/config.ts');
  const hasComment = config.includes('Expo SDK types') && config.includes('channel');
  record(27, 'config.ts cast comment', hasComment ? 'PASS' : 'FAIL',
    hasComment ? 'Inline comment explains type cast' : 'Missing type cast comment');
} catch (e) {
  record(27, 'config.ts cast comment', 'FAIL', e.message);
}

// 28. fly.toml REDIS_URL comment
try {
  const toml = readProjectFile('fly.toml');
  const hasRedisComment = toml.includes('REDIS_URL') && toml.includes('fly secrets');
  record(28, 'fly.toml REDIS_URL comment', hasRedisComment ? 'PASS' : 'FAIL',
    hasRedisComment ? 'REDIS_URL secret documented' : 'Missing REDIS_URL comment');
} catch (e) {
  record(28, 'fly.toml REDIS_URL comment', 'FAIL', e.message);
}
...
let report = `# Validation Report — Run ${RUN} (post /t-review #1)
...
const appendixSection = { name: 'Review Fixes (post /t-review #1)', items: [22, 23, 24, 25, 26, 27, 28] };
report += `## ${appendixSection.name}\n\n`;
for (const id of appendixSection.items) {
  const r = results.find(x => x.id === id);
  if (!r) continue;
  const icon = r.status === 'PASS' ? '✅' : r.status === 'FAIL' ? '❌' : '⏭️';
  report += `${icon} **${r.name}** — ${r.detail}`;
...
const reportPath = resolve(EVIDENCE_DIR, '..', `validation-report-run-${RUN}.md`);
