...
const ASSETS = join(__dirname, '..', 'validation-assets', 'run-7');
...
// ─── Appendix B: Review #2 fixes ─────────────────────────────────────

function testB1_videoPlatformType() {
  try {
    const output = execSync(`grep "VideoPlatform" packages/shared/src/types/index.ts`, { cwd: join(__dirname, '..', '..', '..') }).toString();
    const has5 = output.includes('instagram_account') && output.includes('tiktok_account') && output.includes('manual');
    if (has5) {
      record('Appendix B: Review #2', 'VideoPlatform includes all 5 platforms', 'PASS', 'instagram_account, tiktok_account, manual present');
    } else {
      record('Appendix B: Review #2', 'VideoPlatform includes all 5 platforms', 'FAIL', 'Missing platform values');
    }
  } catch (e) {
    record('Appendix B: Review #2', 'VideoPlatform includes all 5 platforms', 'FAIL', e.message);
  }
}

function testB2_sharedConstants() {
  try {
    const output = execSync(`grep "SUPPORTED_LOCALES\\|SUPPORTED_COUNTRIES" packages/shared/src/constants/locale.ts 2>/dev/null || echo "NOT_FOUND"`, { cwd: join(__dirname, '..', '..', '..') }).toString();
    if (output.includes('SUPPORTED_LOCALES') && output.includes('SUPPORTED_COUNTRIES')) {
      record('Appendix B: Review #2', 'Shared locale constants exist', 'PASS', 'SUPPORTED_LOCALES + SUPPORTED_COUNTRIES in shared package');
    } else {
      record('Appendix B: Review #2', 'Shared locale constants exist', 'FAIL', 'Not found in shared package');
    }
  } catch (e) {
    record('Appendix B: Review #2', 'Shared locale constants exist', 'FAIL', e.message);
  }
}

function testB3_spanishDiacritics() {
  try {
    const output = execSync(`grep "España" packages/shared/src/i18n/es.json`, { cwd: join(__dirname, '..', '..', '..') }).toString();
    if (output.includes('España')) {
      record('Appendix B: Review #2', 'Spanish diacritics present', 'PASS', 'España with ñ found in es.json');
    } else {
      record('Appendix B: Review #2', 'Spanish diacritics present', 'FAIL', 'Missing diacritics');
    }
  } catch (e) {
    record('Appendix B: Review #2', 'Spanish diacritics present', 'FAIL', e.message);
  }
}

// ─── Mobile Fixes ────────────────────────────────────────────────────

function testMF1_videoUnavailableKey() {
  try {
    const es = execSync(`grep "video_unavailable" packages/shared/src/i18n/es.json`, { cwd: join(__dirname, '..', '..', '..') }).toString();
    const en = execSync(`grep "video_unavailable" packages/shared/src/i18n/en.json`, { cwd: join(__dirname, '..', '..', '..') }).toString();
    if (es.includes('video_unavailable') && en.includes('video_unavailable')) {
      record('Mobile Fixes', 'i18n video_unavailable key', 'PASS', 'Key exists in both locale files');
    } else {
      record('Mobile Fixes', 'i18n video_unavailable key', 'FAIL', 'Missing key');
    }
  } catch (e) {
    record('Mobile Fixes', 'i18n video_unavailable key', 'FAIL', e.message);
  }
}

async function testMF2_mp4ReelsExist() {
  const { status, body } = await fetchJson(`${API}/reels?limit=50`);
  if (status !== 200 || !body) return record('Mobile Fixes', 'MP4 reels in feed', 'FAIL', `HTTP ${status}`);
  const mp4Count = body.reels.filter((r) => r.videoType === 'mp4').length;
  if (mp4Count >= 4) {
    record('Mobile Fixes', 'MP4 reels in feed', 'PASS', `${mp4Count} MP4 reels found`);
  } else {
    record('Mobile Fixes', 'MP4 reels in feed', 'FAIL', `Only ${mp4Count} MP4 reels (expected >=4)`);
  }
}

// ─── Appendix C: Review #3 fixes ─────────────────────────────────────

async function testC1_newsCustomAuth() {
  const { status } = await fetchJson(`${API}/news/sources/custom`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ name: 'Test', url: 'https://example.com/rss', sport: 'football', userId: 'x' }),
  });
  if (status === 401) {
    record('Appendix C: Review #3', 'POST /news/sources/custom requires auth', 'PASS', '401 without token');
  } else {
    record('Appendix C: Review #3', 'POST /news/sources/custom requires auth', 'FAIL', `Expected 401, got ${status}`);
  }
}

function testC2_htmlUtilsShared() {
  try {
    const output = execSync(`ls apps/mobile/src/lib/html-utils.ts 2>&1`, { cwd: join(__dirname, '..', '..', '..') }).toString();
    if (output.includes('html-utils.ts')) {
      record('Appendix C: Review #3', 'Shared html-utils module', 'PASS', 'html-utils.ts exists');
    } else {
      record('Appendix C: Review #3', 'Shared html-utils module', 'FAIL', 'File not found');
    }
  } catch (e) {
    record('Appendix C: Review #3', 'Shared html-utils module', 'FAIL', e.message);
  }
}

function testC3_noStickersKey() {
  try {
    const es = execSync(`grep "no_stickers" packages/shared/src/i18n/es.json`, { cwd: join(__dirname, '..', '..', '..') }).toString();
    if (es.includes('no_stickers')) {
      record('Appendix C: Review #3', 'collection.no_stickers i18n key', 'PASS', 'Key exists');
    } else {
      record('Appendix C: Review #3', 'collection.no_stickers i18n key', 'FAIL', 'Missing');
    }
  } catch (e) {
    record('Appendix C: Review #3', 'collection.no_stickers i18n key', 'FAIL', e.message);
  }
}

// ─── PRD3: Google News + Team News ───────────────────────────────────

async function testP3_1_googleNewsSources() {
  const { status, body } = await fetchJson(`${API}/news/sources/catalog`);
  if (status !== 200 || !body) return record('PRD3: Google News', 'News source catalog', 'FAIL', `HTTP ${status}`);
  const sources = body.sources || [];
  const googleNews = sources.filter((s) => s.category === 'google_news');
  if (googleNews.length >= 10) {
    record('PRD3: Google News', 'Google News outlet sources', 'PASS', `${googleNews.length} google_news sources in catalog`);
  } else {
    record('PRD3: Google News', 'Google News outlet sources', 'FAIL', `Only ${googleNews.length} (expected >=10)`);
  }
}

async function testP3_2_teamNewsSources() {
  const { status, body } = await fetchJson(`${API}/news/sources/catalog`);
  if (status !== 200 || !body) return record('PRD3: Team News', 'Team news sources', 'FAIL', `HTTP ${status}`);
  const sources = body.sources || [];
  const teamNews = sources.filter((s) => s.category === 'team_news');
  saveEvidence('api', 'P3-team-news-count.json', { total: teamNews.length, sample: teamNews.slice(0, 5).map(s => s.name) });
  if (teamNews.length >= 100) {
    record('PRD3: Team News', 'Team/athlete news sources', 'PASS', `${teamNews.length} team_news sources (global coverage)`);
  } else {
    record('PRD3: Team News', 'Team/athlete news sources', 'FAIL', `Only ${teamNews.length} (expected >=100)`);
  }
}
...
function generateReport() {
  ...
  let md = `# Validation Report — Run 7 (post /t-review #3)\n\n`;
  md += `**Date**: ${now}\n`;
  ...
  const reportPath = join(ASSETS, '..', 'validation-report-run-7.md');
...
async function main() {
  console.log('🔍 SportyKids Validation — Run 7 (post /t-review #3)\n');
  ...
  // Appendix B: Review #2 fixes
  testB1_videoPlatformType();
  testB2_sharedConstants();
  testB3_spanishDiacritics();

  // Mobile fixes checks
  testMF1_videoUnavailableKey();
  await testMF2_mp4ReelsExist();

  // Appendix C: Review #3 fixes
  await testC1_newsCustomAuth();
  testC2_htmlUtilsShared();
  testC3_noStickersKey();

  // PRD3: Google News + Team News sources
  await testP3_1_googleNewsSources();
  await testP3_2_teamNewsSources();
