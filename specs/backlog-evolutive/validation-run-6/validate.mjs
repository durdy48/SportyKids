...
const ASSETS = join(__dirname, '..', 'validation-assets', 'run-6');
...
function generateReport() {
  ...
  let md = `# Validation Report — Run 6 (PRD1+2+3 + all fixes)\n\n`;
  md += `**Date**: ${now}\n`;
  ...
  const reportPath = join(ASSETS, '..', 'validation-report-run-6.md');
...
async function main() {
  console.log('🔍 SportyKids Validation — Run 6 (PRD1+2+3 + all fixes)\n');
