#!/bin/bash
# =============================================================================
# Validation Run 12 — prd6.md: Production Readiness (PostgreSQL, Redis, OAuth)
# =============================================================================
set -uo pipefail

API_BASE="http://localhost:3001/api"
SPEC_DIR="specs/backlog-evolutive"
ASSETS_DIR="$SPEC_DIR/validation-assets/run-12"
API_DIR="$ASSETS_DIR/api"
OUTPUT_DIR="$ASSETS_DIR/output"
REPORT_FILE="$SPEC_DIR/validation-assets/validation-report-run-12.md"
...
if [ "$HTTP_CODE" = "501" ]; then
  if echo "$BODY" | grep -q '"provider":"google"'; then
    record "PASS" "AC-OA1: GET /auth/google" "Returns 501 with provider=google — [payload](run-12/api/oauth-google.json)"
  else
...
if [ "$HTTP_CODE" = "501" ]; then
  if echo "$BODY" | grep -q '"provider":"apple"'; then
    record "PASS" "AC-OA2: GET /auth/apple" "Returns 501 with provider=apple — [payload](run-12/api/oauth-apple.json)"
  else
...
if echo "$TEST_OUTPUT" | grep -q "passed" && ! echo "$TEST_OUTPUT" | grep -q "failed"; then
  record "PASS" "Full test suite" "$TEST_FILES — [output](run-12/output/test-suite.txt)"
else
  record "FAIL" "Full test suite" "Some tests failed — [output](run-12/output/test-suite.txt)"
fi
...
echo ""
echo "=== Validation Run 12 — prd6.md ==="
